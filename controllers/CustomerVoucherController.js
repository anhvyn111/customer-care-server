...
router.post('/', auth.isStaff, async (req, res) => {
    try{
        ...
        var isCustomerVoucherExisted = await _voucherService.isCustomerVoucherExisted(voucher._id, customer._id);
        if (isCustomerVoucherExisted == true){
            return res.status(400).json("This customer already had this voucher.");
        }

        var newcustomerVoucher = {voucherId, customerId, dueDate: new Date().setDate(voucher.duration)};
        newCustomerVoucher = await _voucherService.createCustomerVoucher(newcustomerVoucher);
